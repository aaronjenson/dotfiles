return {
  ...
  config = function()
    ...
    setup_server('angularls', {
      settings = {
        root_dir = require('lspconfig.util').root_pattern('tslint.json')
      }
    })
    ...
    setup_server('ruby_lsp', {
      cmd = { 'mise', 'x', '--', 'dotenv', '-i', '-f', '.env.lsp', 'ruby-lsp' },
      init_options = {
        linters = {'rubocop'},
      }
    })

    -- Already used by ruby_lsp, and conflicts if both are running
    -- setup_server('rubocop', {
